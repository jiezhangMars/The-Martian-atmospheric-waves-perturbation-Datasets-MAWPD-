...
# float64 amp_T_DE1(Latitude, Altitude, Solar_Longitude), float64 amp_T_DW2(Latitude, Altitude, Solar_Longitude),
...
amp_T_DE1=nc_obj['amp_T_DE1'][:].data
phs_T_DW1=nc_obj['phs_T_DW1'][:].data

amp_T_DE1=np.transpose(amp_T_DE1,(2,1,0))
...
for i in range(round(72/6)):
    Temperature_to_plot=np.squeeze(np.nanmean(amp_T_DE1[i*6:(i+1)*6,:,:],axis=(0)))

    Temperature_to_plot[Temperature_to_plot>8]=8
    ...
    ax[i].set_title('('+chr(i + 97)+') Ls='+str(i*30)+'-'+str((i+1)*30)+' at 30 km', x=0.06+0.18,fontdict={'weight': 'heavy', 'size': 15,'family':'Times New Roman'})
...
# plt.savefig('T_5_re.png',dpi=600,bbox_inches = 'tight')
